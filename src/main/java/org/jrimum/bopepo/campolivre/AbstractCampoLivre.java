		...
		try{
			...
			if (BancosSuportados.isSuportado(titulo.getContaBancaria().getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado())) {
				...
				switch (banco) {
					...
					case BANCO_INTER: return AbstractCLBancoInter.create(titulo);
					...
					case BANCO_B2S: return AbstractCLBancoB2s.create(titulo);
					case UNICRED: return AbstractCLUnicred.create(titulo);
					...
					case VORTX: return AbstractCLVortx.create(titulo);
